...
const handleOfficeSelection = (officeId) => {
    ...
    axios.get(`${API_BASE_URL}/targets-by-office?office_id=${officeId}`)
        .then(response => {
            console.log("📊 Targets for Selected Office:", response.data);
            setTargets(response.data);
        })
        .catch(error => console.error("❌ Error fetching targets:", error));
};

const handleOfficeSelection_old = () => {
    let officeId = selectedSubdivision || selectedDivision || selectedCircle;
    ...
    const handleEmployeeChange = (e) => {
        ...
        if (!employeeId) {
            ...
            //handleOfficeSelection(selectedSubdivision || selectedDivision || selectedCircle);
            return;
        }
         axios.get(`${API_BASE_URL}/targets?employee_id=${employeeId}`)
        .then(response => {
            setTargets(response.data);
        })
        .catch(error => console.error("❌ Error fetching targets for employee:", error));
