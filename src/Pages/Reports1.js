...
const Reports = () => {
        ...
        datasets: [
            {
                ...
                data: months.map(month => reportData.filter(d => d.month === month).reduce((sum, d) => sum + d.target_value, 0)),
                backgroundColor: "rgba(54, 162, 235, 0.6)",
            },
            {
                label: "Achieved Target",
                data: months.map(month => reportData.filter(d => d.month === month).reduce((sum, d) => sum + d.achieved_value, 0)),
    ...
    const lineChartData = {
        ...
        datasets: roles.map(role => ({
            ...
            data: months.map(month => reportData.filter(d => d.month === month && d.role === role).reduce((sum, d) => sum + d.achieved_value, 0)),
    ...
    const roleWiseData = roles.map(role => ({
        role,
        totalAchieved: reportData.filter(d => d.role === role).reduce((sum, d) => sum + d.achieved_value, 0)
    }));
